# -*- coding: utf-8 -*-
"""
基于训练集的语义均值作为该类的正确语义，计算准确率、夹角分布、混淆矩阵
"""
import os, sys, math
import argparse
...
class Metrics:
    ...
    def debug(self):
        ...
        histplot_angles([angles.reshape([-1])], ['train_angles'], 'train_angles distribution')
    ...
    def build(self):
        ...
        # 得到语义向量 - 已l2规范化
        self.train_emb, _, self.train_uemb = get_emb(self.train_imgs, name='arcface', reuse=False, training=False,
                                                     keep_prob=1, summary=False, get_unl2=True)

        _args = [self.train_emb, self.input_emb_average, self.train_labels]
        ...
        # 得到语义向量 - 已l2规范化
        emb, _, self.test_uemb = get_emb(self.test_imgs, name='arcface', reuse=True, training=False,
                                         keep_prob=1, summary=False, get_unl2=True)

        _args = [emb, self.input_emb_average, self.test_labels]
    ...
    def com_all(self):
        ...
        restore_path = os.path.join(self.save_dir, 'metrics2_emb_average.npy')
        try:
            emb_average = np.load(restore_path)
            print(f'SUCCEED: load emb_average from {restore_path}.')
        except:
            emb_average = self.com_emb_average()

        self._com_all('train', emb_average)
        self._com_all('test', emb_average)

    def _com_all(self, cate, emb_average):
        if cate == 'train':
        ...
        while True:
            try:
                res = self.sess.run(rum_list, {self.input_emb_average: emb_average})
        ...
        np.save(os.path.join(self.save_dir, f'metrics2_{cate}_acc.npy'), acc)
        np.save(os.path.join(self.save_dir, f'metrics2_{cate}_angles1.npy'), cum_angles1)
        np.save(os.path.join(self.save_dir, f'metrics2_{cate}_angles2.npy'), cum_angles2)
        # 画图
        save_path = os.path.join(self.save_dir, f'metrics2_{cate}_angle_distribution.png')
        histplot_angles([cum_angles1, cum_angles2], ['angles1', 'angles2'],
                        title=f'metrics2_{cate} angle distribution', show=True, save_path=save_path)

        # 混淆矩阵
        confusion_matrix = np.zeros([config.num_cls, config.num_cls])
        lp = list(zip(cum_labels, cum_predict))
        np.save(os.path.join(self.save_dir, f'metrics2_{cate}_label_predict'), np.array(lp))

        rcn = np.array([[i[0], i[1], lp.count(i)] for i in set(zip(cum_labels, cum_predict))])
        confusion_matrix[rcn[:, 0], rcn[:, 1]] += rcn[:, 2]
        np.save(os.path.join(self.save_dir, f'metrics2_{cate}_confusion_matrix'), confusion_matrix)
        print(f'\nSUCCEED get_confusion_matrix from {cate}.')

        # 画出混淆矩阵
        plt.figure(figsize=[10, 10])
        sns.heatmap(confusion_matrix)
        plt.title(f'metrics2_{cate} confusion_matrix')
        plt.xlim([-0.5, config.num_cls + 0.5])
        plt.ylim([-0.5, config.num_cls + 0.5])
        plt.savefig(os.path.join(self.save_dir, f'metrics2_{cate}_confusion_matrix.png'))
        plt.show()

        # 评价混淆矩阵
        self.metrics_cm(confusion_matrix, f'metrics2_{cate}')
    ...
    def com_emb_average(self):
        ...
        save_path = os.path.join(self.save_dir, 'metrics2_emb_average.npy')
